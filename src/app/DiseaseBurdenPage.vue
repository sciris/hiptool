...
export default {
  ...
  methods: {
    ...
    notImplemented() {
      sciris.fail(this, 'This feature is not implemented')
    },
    ...
    updateBurdenSets(setLastEntryActive) {
      ...
      if (this.$store.state.activeProject.project === undefined) { // If there is no active project, clear the burdenSets list.
        ...
        sciris.rpc('jsonify_burdensets', [this.$store.state.activeProject.project.id]) // Get the active project's burden sets.
        .then(response => {
    ...
    viewBurdenSet(burdenSet, verbose) {
      ...
      if (verbose) {
        sciris.succeed(this, 'Burden set "' + burdenSet.burdenset.name + '" now active')
      }
    },
    ...
    makeGraph(burdenSet) {
      ...
      this.clearGraphs()
      sciris.rpc('plot_burden',  // Go to the server to get the diseases from the burden set.
        [
          this.$store.state.activeProject.project.id, 
          this.activeBurdenSet.burdenset.numindex
        ])
    ...
    uploadBurdenSet(burdenSet) {
      ...
      .then(response => {
        ...
        sciris.succeed(this, 'Burden set uploaded')
      })
      .catch(error => {
        sciris.fail(this, 'Could not upload burden set', error)
      })
    },
    ...
    downloadBurdenSet(burdenSet) {
      ...
      .catch(error => {
        sciris.fail(this, 'Could not download burden set', error)
      })
    },
    ...
    deleteBurdenSet(burdenSet) {
      ...
      sciris.rpc('delete_set', [this.$store.state.activeProject.project.id, 'burdenset', burdenSet.burdenset.numindex]) // Go to the server to delete the burden set.
      .then(response => {
    ...
    createNewBurdenSet() {
      ...
      sciris.rpc('create_burdenset', [this.$store.state.activeProject.project.id, this.country]) // Go to the server to create the new burden set.
        .then(response => {
        this.updateBurdenSets(true) // Update the burden sets so the new set shows up on the list.
      })
    },
    ...
    updateDisease(disease) {
      ...
      sciris.rpc('update_disease', [this.$store.state.activeProject.project.id, this.activeBurdenSet.burdenset.numindex, disease.numindex,
        [disease['Active'], disease['Cause'], disease['DALYs'], disease['Deaths'], disease['Prevalence']]])
      .then(response => {
        sciris.succeed(this, 'Burden set updated')
      })
        .catch(error => {
          sciris.fail(this, 'Could not update burden set', error)
        })
    },

    addBurden() {
      console.log('Adding burden')
      sciris.rpc('add_burden', [this.$store.state.activeProject.project.id, this.activeBurdenSet.burdenset.numindex])
        .then(response => {
          this.viewBurdenSet(this.activeBurdenSet) // Update the display of the burden list by rerunning the active burden set.
          sciris.succeed(this, 'Burden added')
        })
        .catch(error => {
          sciris.fail(this, 'Could not add burden', error)
        })
    },

    copyBurden(burden) {
      console.log('Item to copy:', burden.numindex)
      sciris.rpc('copy_burden', [this.$store.state.activeProject.project.id, this.activeBurdenSet.burdenset.numindex, burden.numindex])
        .then(response => {
          this.viewBurdenSet(this.activeBurdenSet) // Update the display of the burden list by rerunning the active burden set.
          sciris.succeed(this, 'Burden copied')
        })
        .catch(error => {
          sciris.fail(this, 'Could not copy burden', error)
        })
    },

    deleteBurden(burden) {
      console.log('Item to delete:', burden.numindex)
      sciris.rpc('delete_burden', [this.$store.state.activeProject.project.id, this.activeBurdenSet.burdenset.numindex, burden.numindex])
        .then(response => {
          this.viewBurdenSet(this.activeBurdenSet) // Update the display of the burden list by rerunning the active burden set.
          sciris.succeed(this, 'Burden deleted')
        })
        .catch(error => {
          sciris.fail(this, 'Could not delete burden', error)
        })
    },
