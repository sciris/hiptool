...
export default {
  ...
  methods: {
    ...
    clearGraphs(numfigs) { return sciris.clearGraphs(this, numfigs)},
    ...
    downloadPlots() {
      sciris.download('download_figures', [this.$store.state.currentUser.username])
        .then(response => {
    ...
    optimize() {
      ...
      sciris.rpc('optimize', [this.$store.state.activeProject.project.id, this.activePackageSet.packageset.numindex, this.budget, this.frpwt, this.equitywt, fixedList])
        .then(response => {
          console.log('Optimized')
          this.updatePackageSets(false)
          this.viewPackageSet(this.activePackageSet)
          sciris.succeed(this, 'Optimizing...')
        })
    },
    ...
    updatePackageSets(setLastEntryActive) {
      ...
      if (this.$store.state.activeProject.project === undefined) { // If there is no active project, clear the packageSets list.
        ...
        sciris.rpc('jsonify_packagesets', [this.$store.state.activeProject.project.id]) // Get the active project's health package sets.
          .then(response => {
          ...
          .catch(error => {
            sciris.fail(this, 'Could not get package sets', error)
          })
      }
    },
    ...
    viewPackageSet(packageSet, verbose) {
      ...
      sciris.rpc('jsonify_packages', [this.$store.state.activeProject.project.id, this.activePackageSet.packageset.numindex]) // Go to the server to get the diseases from the package set.
        .then(response => {
        ...
        .catch(error => {
          sciris.fail(this, 'Could not view health package sets', error)
        })
      if (verbose) {
        sciris.succeed(this, 'Health package "' + packageSet.packageset.name + '" now active')
      }
    },
    ...
    makeGraph(packageSet) {
      ...
      .then(response => {
        ...
        var index = 1;
        _.forOwn(this.graphData, function(graph){
          mpld3.draw_figure('fig'+index, graph) // Draw the figure.
          index ++;
        })
      })
      /**
        .catch(error => {
          sciris.fail(this, 'Could not make graphs', error)
        })
      **/
    },
    ...
    copyPackageSet(packageSet) {
      ...
      sciris.rpc('copy_set', [this.$store.state.activeProject.project.id, 'packageset', packageSet.packageset.numindex]) // Have the server copy the package set, giving it a new name.
        .then(response => {
    ...
    uploadPackageSet(packageSet) {
      ...
      sciris.upload('upload_set', [this.$store.state.activeProject.project.id, 'packageset', packageSet.packageset.numindex], {}, '.xlsx')
        .then(response => {
          this.updatePackageSets(true)
          sciris.succeed(this, 'Package set uploaded')
        })
        .catch(error => {
          sciris.fail(this, 'Could not upload package set', error)
        })
    },

    downloadPackageSet(packageSet) {
      console.log('downloadPackageSet() called for ' + packageSet.packageset)
      sciris.download('download_set', [this.$store.state.activeProject.project.id, 'packageset', packageSet.packageset.numindex])
        .then(response => {
    ...
    renamePackageSet(packageSet) {
      ...
      if (packageSet.renaming === '') { // If the package set is not in a mode to be renamed, make it so.
        ...
        sciris.rpc('rename_set', [this.$store.state.activeProject.project.id, 'packageset', packageSet.packageset.numindex, packageSet.renaming]) // Have the server change the name of the package set.
          .then(response => {
    ...
    deletePackageSet(packageSet) {
      ...
      sciris.rpc('delete_set', [this.$store.state.activeProject.project.id, 'packageset', packageSet.packageset.numindex]) // Go to the server to delete the package set.
        .then(response => {
          this.updatePackageSets() // Update the package sets so the new set shows up on the list.
        })
    },
    ...
    createNewPackageSet() {
      ...
      sciris.rpc('create_packageset', [this.$store.state.activeProject.project.id, this.burdenSet, this.intervSet]) // Go to the server to create the new package set.
        .then(response => {
          this.updatePackageSets(true) // Update the package sets so the new set shows up on the list.
        })
        .catch(error => {
          sciris.fail(this, 'Could not create new package sets', error)
        })
    },
